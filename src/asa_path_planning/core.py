...
# Core geometry, scenario, and algorithm implementations. Experiment orchestration
# lives in suites.py; imports from experiment.py remain backward compatible.
...
def rrt_star(
    ...
    for _ in range(max_iter):
        ...
        if not is_free_point(scenario, new_point):
            continue
        if not line_free(scenario, nodes[nearest_index], new_point, step=0.5):
            continue

        near_indices = [
            i
            for i, point in enumerate(nodes)
            if math.dist(point, new_point) <= radius
            and line_free(scenario, point, new_point, step=0.5)
        ]
        ...
        for i in near_indices:
            ...
            if new_cost < cost[i] and line_free(scenario, new_point, nodes[i], step=0.5):
                parent[i] = new_index
                cost[i] = new_cost

        if math.dist(new_point, goal) <= step_size and line_free(
            scenario, new_point, goal, step=0.5
        ):
